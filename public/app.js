...
function checkAccessAndRoute() {
  ...
  if (!hasValidAccess() && joinCode) {
    ...
    if (typeof checkAndShowGlobalAdWarning === 'function') checkAndShowGlobalAdWarning(); // Call on payment
          window.history.replaceState({}, document.title, window.location.pathname);
  ...
  if (!hasValidAccess()) {
    ...
    getPlatformConfig().then(config => {
      ...
      if (typeof checkAndShowGlobalAdWarning === 'function') checkAndShowGlobalAdWarning(); // Call on dash
...
function showRoom(roomId, url = null, file = null) {
  ...
  // Show Ad Instruction Banner for free users
  const adBanner = document.getElementById('ad-instruction-banner');
  if (adBanner && typeof shouldShowInMovieAds === 'function' && shouldShowInMovieAds()) {
...
function detectAdBlock() {
  ...
  bait.className = 'ad-banner pub_300x250 pub_728x90 text-ad ad-slot';
  ...
  setTimeout(() => {
    ...
    if (isBlocked) {
      const paidPlans = ['weekly', 'monthly', 'access-code', 'extended_by_admin', 'one-time'];
      const isPaid = currentUserDoc && paidPlans.includes(currentUserDoc.activeSubscription) && currentUserDoc.subscriptionExpiry > Date.now();
      if (isPaid || (currentUser && currentUser.email === 'anubhabmohapatra.01@gmail.com')) {
        bait.remove();
        return;
      }
      showAdBlockWarning();
...
function showAdBlockWarning() {
  ...
  overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:#09090b;z-index:9999999;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#fff;text-align:center;padding:2rem;font-family:Inter,sans-serif;';
  overlay.innerHTML = `
        <div style="font-size:5rem;margin-bottom:1rem;">??</div>
        <h1 style="font-size:2rem;font-weight:800;color:#ef4444;margin-bottom:1rem;">Adblocker Detected</h1>
        <p style="font-size:1.1rem;color:#a1a1aa;max-width:500px;line-height:1.6;margin-bottom:2rem;">
            Vaanisethu relies on ads to keep the platform <b>100% free</b> for you.<br><br>
            Please disable your Adblocker or whitelist this site to continue watching movies. Once disabled, refresh the page.
        </p>
        <button onclick="window.location.reload()" style="padding:0.8rem 2rem;background:#ef4444;color:#fff;border:none;border-radius:8px;font-size:1.1rem;font-weight:600;cursor:pointer;">I have disabled it, Refresh Page</button>
    `;
  document.body.appendChild(overlay);
}
setInterval(detectAdBlock, 10000);
setTimeout(detectAdBlock, 3000);



// --- Global Ad Warning Logic ---
window.checkAndShowGlobalAdWarning = function() {
  const adBanner = document.getElementById('ad-instruction-banner');
  if (!adBanner) return;
  
  // Only show to normal users (who see ads)
  if (typeof shouldShowInMovieAds === 'function' && shouldShowInMovieAds()) {
    
    // Check if seen today
    const today = new Date().toISOString().split('T')[0];
    const lastSeen = localStorage.getItem('adWarningLastSeenDate');
    
    // Check if Select Source is active
    const sourceUI = document.getElementById('source-ui');
    const isSourceUIVisible = sourceUI && !sourceUI.classList.contains('hidden');

    if (lastSeen !== today || isSourceUIVisible) {
      adBanner.classList.remove('hidden');
      adBanner.style.display = 'flex';
      localStorage.setItem('adWarningLastSeenDate', today);
    }
  }
};
